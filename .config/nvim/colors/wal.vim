" Vim Color File
" Name: wal.vim
" Based On: embark.vim (Challenger Deep, Ayu Mirage, Manta), repalettised to
"           consume pywal's ~/.cache/wal/colors-wal.vim at load time.
...
if !exists("g:wal_terminal_italics")
  let g:wal_terminal_italics = 0
endif

if !exists("g:wal_termcolors")
  let g:wal_termcolors = 256
endif

" == SOURCE PYWAL COLORS ==
"
" pywal writes colors-wal.vim as a set of *unscoped* `let`s, which become
" globals (g:background, g:foreground, g:cursor, g:color0..15) once sourced.
let s:wal = expand('~/.cache/wal/colors-wal.vim')
if filereadable(s:wal)
  execute 'source' s:wal
endif

" Fallback so the file never errors if the cache is missing (tokyonight-moon).
if !exists('g:background') | let g:background = '#24283b' | endif
if !exists('g:foreground') | let g:foreground = '#c0caf5' | endif
if !exists('g:color0')     | let g:color0  = '#1a1b26' | endif
if !exists('g:color1')     | let g:color1  = '#f7768e' | endif
if !exists('g:color2')     | let g:color2  = '#9ece6a' | endif
if !exists('g:color3')     | let g:color3  = '#e0af68' | endif
if !exists('g:color4')     | let g:color4  = '#7aa2f7' | endif
if !exists('g:color5')     | let g:color5  = '#ad8ee6' | endif
if !exists('g:color6')     | let g:color6  = '#449dab' | endif
if !exists('g:color7')     | let g:color7  = '#787c99' | endif
if !exists('g:color8')     | let g:color8  = '#bb9af7' | endif
if !exists('g:color9')     | let g:color9  = '#ff7a93' | endif
if !exists('g:color10')    | let g:color10 = '#b9f27c' | endif
if !exists('g:color11')    | let g:color11 = '#ff9e64' | endif
if !exists('g:color12')    | let g:color12 = '#7da6ff' | endif
if !exists('g:color13')    | let g:color13 = '#bb9af7' | endif
if !exists('g:color14')    | let g:color14 = '#0db9d7' | endif
if !exists('g:color15')    | let g:color15 = '#acb0d0' | endif

" == COLOR PALETTE ==
"
" Every value is mapped from the pywal 16-color palette + bg/fg.
" `cterm`/`cterm16` use the matching ANSI index (0-15) so non-truecolor
" sessions reuse the terminal's own (also pywal-set) colors.
"
" A flat 16-color palette has fewer surface tones than embark's original, so
" the dark surfaces (CursorLine / Visual / Pmenu / ColorColumn) all collapse
" onto color0. That's the expected trade for a wal-derived scheme.

" Surfaces (dark -> light)
let s:space0 = { "gui": g:color0,     "cterm": "0",    "cterm16": "0"    }
let s:space1 = { "gui": g:background, "cterm": "NONE", "cterm16": "NONE" }
let s:space2 = { "gui": g:color0,     "cterm": "0",    "cterm16": "0"    }
let s:space3 = { "gui": g:color0,     "cterm": "0",    "cterm16": "0"    }
let s:space4 = { "gui": g:color7,     "cterm": "7",    "cterm16": "7"    }

" Foregrounds
let s:astral0 = { "gui": g:color7,     "cterm": "7",  "cterm16": "7"  }
let s:astral1 = { "gui": g:foreground, "cterm": "15", "cterm16": "7"  }

" Accents (normal / bright pairs)
let s:red         = { "gui": g:color1,  "cterm": "1",  "cterm16": "1"  }
let s:dark_red    = { "gui": g:color9,  "cterm": "9",  "cterm16": "9"  }

let s:green       = { "gui": g:color2,  "cterm": "2",  "cterm16": "2"  }
let s:dark_green  = { "gui": g:color10, "cterm": "10", "cterm16": "10" }

let s:yellow      = { "gui": g:color3,  "cterm": "3",  "cterm16": "3"  }
let s:dark_yellow = { "gui": g:color11, "cterm": "11", "cterm16": "11" }

let s:blue        = { "gui": g:color4,  "cterm": "4",  "cterm16": "4"  }
let s:dark_blue   = { "gui": g:color12, "cterm": "12", "cterm16": "12" }

let s:purple      = { "gui": g:color5,  "cterm": "5",  "cterm16": "5"  }
let s:dark_purple = { "gui": g:color13, "cterm": "13", "cterm16": "13" }

let s:cyan        = { "gui": g:color6,  "cterm": "6",  "cterm16": "6"  }
let s:dark_cyan   = { "gui": g:color14, "cterm": "14", "cterm16": "14" }
...
" Diff backgrounds are the only non-wal-derived values: a flat 16-color
" palette has no muted add/change/delete tints, so these stay as fixed dark
" hues. Swap them for color2/color1/color3 if you prefer loud full-saturation.
